function save(io::IO, sfrm::SiemensFrame)
    ...
    write(io, @sprintf "RANGE  :%-72f" abs(rad2deg(sfrm.increment)))
    write(io, @sprintf "START  :%-72f" rad2deg(sfrm.angles[sfrm.axis]))
    write(io, @sprintf "INCREME:%-72f" rad2deg(sfrm.increment))
    write(io, @sprintf "NUMBER :%-72d" 0)
    write(io, @sprintf "NFRAMES:%-72d" 1)
    write(io, @sprintf "ANGLES :%-17f %-17f %-17f %-17f " rad2deg.(sfrm.angles)...)
    ...
    write(io, @sprintf "ENDING :%-17f %-17f %-17f %-17f " rad2deg.(ending)...)
